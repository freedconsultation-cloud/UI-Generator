...
// because the Vercel AI SDK models a single turn as an array of typed parts.
          ...
          >
            ...
            <div className={cn(
              ...
              <div className={cn(
                ...
                // Blue bubble for user messages; white card for assistant messages
                message.role === "user"
                  ? "bg-blue-600 text-white shadow-sm"
                  : "bg-white text-neutral-900 border border-neutral-200 shadow-sm"
              )}>
                <div className="text-sm">
                  {/* Render using the parts array when available (newer AI SDK format) */}
                  {message.parts ? (
                    <>
                      {message.parts.map((part, partIndex) => {
                        switch (part.type) {
                          // Plain text — render as markdown for assistant, plain span for user
                          case "text":
                          ...
                          // Tool call badge — delegates rendering to ToolCallBadge
                          // which maps tool names and args to human-readable labels
                          ...
                          // Source citation (not commonly triggered in this app)
                          case "source":
                            return (
                          ...
                          // Visual separator between multi-step reasoning turns
                          case "step-start":
                      ...
                      {/* Streaming indicator appended to the last assistant message */}
                      {isLoading &&
                        message.role === "assistant" &&
                    ...
                    // Fallback for older messages that only have a content string
                    message.role === "user" ? (
