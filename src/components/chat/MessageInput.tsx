...
// MessageInput — the textarea + send button at the bottom of the chat panel.
// Submits the form on Enter (without Shift), and on explicit button click.
...
export function MessageInput({
  ...
  // Submit the form when Enter is pressed alone; allow Shift+Enter for newlines
  const handleKeyDown = (e: KeyboardEvent<HTMLTextAreaElement>) => {
    if (e.key === "Enter" && !e.shiftKey) {
      e.preventDefault();
      // Use requestSubmit so the form's onSubmit handler fires (not just a click)
  ...
  return (
    <form onSubmit={handleSubmit} className="relative p-4 bg-white border-t border-neutral-200/60">
      <div className="relative max-w-4xl mx-auto">
        {/* Auto-resizing textarea with a min/max height range */}
        <textarea
          value={input}
          ...
          rows={3}
          // Grammarly and similar extensions inject data-gramm* attributes onto
          // textareas before React hydrates — suppress the mismatch warning
          suppressHydrationWarning
        />

        {/* Send button — disabled when loading or when the input is empty/whitespace-only */}
        <button
          type="submit"
        ...
        >
          {/* Icon color changes to blue when the button is active */}
